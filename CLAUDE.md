...
## Known gotchas
- **`ScrollView`s clip silently when their height is ambiguous.** Horizontal ones collapse vertically (clipped pill descenders) — give them an explicit `height`. A percentage `maxHeight` inside a content-sized parent does the same: the goal sheet cut off its last row *and* refused to scroll until the ScrollView was dropped entirely.
