...
- `src/stretches/library.ts` → imports `library.data.json` — **82 stretches GENERATED** from the public-domain [exercises dataset](https://github.com/yuhonas/free-exercise-db) by `scripts/build-library.ts`. Edit content via the script, not by hand. Photos in `assets/stretches/<id>/{0,1}.jpg`. Noise (foam rolling, partner stretches, rep-based movements) is pruned by `src/stretches/prune.ts` — dropped entries are recorded in `src/stretches/DROPPED.md`; rescue one via the `KEEP` set in `prune.ts` + a re-run.
