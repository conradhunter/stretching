...
- `src/exercises/log.ts` — **pure** per-day rep counts (`recordReps`/`repsOn`/`dayTotal`). `store.ts` — persists them under `exercises.v1`. Kept **separate from `src/tracking`**: reps are counted, never timed, and never feed the daily goal or streak.
...
- `src/components/goal-ring.tsx` — the streak ring (Quick header); tap opens the goal sheet, long-press opens `/debug`. `src/audio/chime.ts` — segment chime (audio mode set to `mixWithOthers` so it never pauses the user's music).
