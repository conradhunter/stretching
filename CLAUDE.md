...
- `src/components/goal-ring.tsx` — the streak ring (Quick + Exercises headers); tap opens the goal sheet (minutes + optional per-exercise rep targets), long-press opens `/debug`. The ring's **fill is stretch time**, but it only **closes when the whole goal is met** — a finished 15 minutes with push-ups still owed keeps its visible gap, or the ring would repeat the Aug 13 false "done". `src/audio/chime.ts` — segment chime (audio mode set to `mixWithOthers` so it never pauses the user's music).
